    ...
    return (
        <div className="w-5/6 mt-10 mx-auto flex flex-col">
            ...
            <textarea name="notes" id="notes" rows="3"
                className="border-2 border-gray-700 w-1/3 shadow"
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
            />
            {error ? <h1 className="text-red-500">{error}</h1> : null}
            <button className="border-2 rounded-xl border-green-500 bg-green-500 text-white font-semibold p-1 ml-1 mt-2 w-20 shadow" onClick={submitNotes}>Save</button>
        </div>
    );
}
